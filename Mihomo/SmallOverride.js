...
const args = {
  ...
  regionSet: "none", // 匹配 regionDefinitions.name 前两个字母 (严格大小写)
...
let {
  ...
  ruleSet = args.ruleSet || "openai;youtube;ads", // 支持 'all' 或 'openai,youtube,ads' 这种格式
...
function main(config) {
  ...
  functionalGroups.push({
    ...
    name: "所有节点",
    type: "url-test",
    interval: 60, // 自动测试间隔（秒）
    timeout: 1500,
    tolerance: 50, // 容差值（单位 ms），若最低延迟与当前节点差距小于50ms，则不切换
    proxies: [...allProxies],
  ...
  serviceConfigs.forEach((svc) => {
    if (ruleOptions[svc.key]) {
      ...
      if (svc.reject) {
        ...
        groupProxies = ["所有节点", "直连", ...regionGroupNames];
      } else {
        groupProxies = ["所有节点", ...regionGroupNames, "直连"];
      }
  ...
  functionalGroups.push(
    ...
    },
    {
      ...
      name: "其他外网",
      type: "select",
      proxies: ["所有节点", "默认节点", "国内网站", ...regionGroupNames],
    ...
    },
    {
      ...
      name: "国内网站",
      type: "select",
      proxies: ["直连", "所有节点", "默认节点", ...regionGroupNames],
